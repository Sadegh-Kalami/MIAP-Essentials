...
sigma_10 = 75;
[denoised_10db, ~] = denoiseImageDCT(noisy_temp_10db, sigma_10, num_atoms);
psnr_denoised_10db = 20 * log10(255 / sqrt(mean((denoised_10db(:) - temp_img(:)).^2)));

noisy_temp_15db = noisy_15db * 255;
sigma_15 = 45;
[denoised_15db, ~] = denoiseImageDCT(noisy_temp_15db, sigma_15, num_atoms);
psnr_denoised_15db = 20 * log10(255 / sqrt(mean((denoised_15db(:) - temp_img(:)).^2)));

noisy_temp_20db = noisy_20db * 255;
sigma_20 = 25;
[denoised_20db, ~] = denoiseImageDCT(noisy_temp_20db, sigma_20, num_atoms);
...
subplot(3, 3, 3); imshow(denoised_10db / 255); title(['DCT (PSNR: ', num2str(psnr_denoised_10db), ')']);

subplot(3, 3, 4); imshow(image_data); title('Original');
subplot(3, 3, 5); imshow(noisy_15db); title(['Noisy (PSNR: ', num2str(psnr_15db), ')']);
subplot(3, 3, 6); imshow(denoised_15db / 255); title(['DCT (PSNR: ', num2str(psnr_denoised_15db), ')']);

subplot(3, 3, 7); imshow(image_data); title('Original');
subplot(3, 3, 8); imshow(noisy_20db); title(['Noisy (PSNR: ', num2str(psnr_20db), ')']);
subplot(3, 3, 9); imshow(denoised_20db / 255); title(['DCT (PSNR: ', num2str(psnr_denoised_20db), ')']);
